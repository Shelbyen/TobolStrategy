...
public class CameraController : MonoBehaviour
{
    ...
    {
        float Rotation = 0;
        if (Input.GetMouseButton(2)) Rotation = Input.GetAxis("Mouse X") * Sensitivity;
        else if (Input.GetKey("q")) Rotation = 1;
        else if (Input.GetKey("e")) Rotation = -1;
        if (Input.GetMouseButtonDown(2)) Cursor.lockState = CursorLockMode.Locked;
        if (Input.GetMouseButtonUp(2)) Cursor.lockState = CursorLockMode.Confined;
    ...
    private void CameraZooming()
    {
        ...
        CameraZoomer.transform.localPosition = new Vector3(0, 0, Mathf.Clamp(CameraZoomer.transform.localPosition.z, -10, 0));
    }

    private void CameraTilt()
    {
        float Tilt = 0;
        if (Input.GetMouseButton(2)) Tilt = Input.GetAxis("Mouse Y");
